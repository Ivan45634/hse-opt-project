...
def squad_preprocess(data_args, training_args, model_args):
    ...
    if torch.cuda.get_device_capability()[0] >= 8:
        # attn_implementation = "flash_attention_2" [TODO add fix]
        attn_implementation = "eager"
        torch_dtype = torch.bfloat16
    else:
        attn_implementation = "eager"
        torch_dtype = torch.float16
    # Remove BitsAndBytesConfig as it might not be compatible with AutoModelForQuestionAnswering
        ...
        for i, offsets in enumerate(offset_mapping):
            ...
            if len(answers["answer_start"]) == 0:
                ...
                if not (offsets[token_start_index][0] <= start_char and offsets[token_end_index][1] >= end_char):
                    ...
                    # Add a check to ensure token_end_index is within bounds
